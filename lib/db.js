...
// ─── Physical Store & Consigned Stock Helpers ─────────────────────────────────
async function getStores() {
  await initDB();
  const { rows } = await pool.query(`
    SELECT s.id, s.name, s.location, s.contact, s.notes, s.created_at AS "createdAt",
           COALESCE(COUNT(ss.id), 0)::int AS "itemTypesCount",
           COALESCE(SUM(ss.quantity), 0)::int AS "totalUnits",
           COALESCE(SUM(ss.quantity * ss.price), 0)::float AS "totalValuation"
    FROM stores s
    LEFT JOIN store_stock ss ON s.id = ss.store_id
    GROUP BY s.id
    ORDER BY s.name ASC
  `);
  return rows;
...
async function getStoreById(id) {
  ...
  const storeRes = await pool.query(`
    SELECT id, name, location, contact, notes, created_at AS "createdAt"
    FROM stores WHERE id = $1
  `, [id]);
  if (!storeRes.rows[0]) return null;

  const stockRes = await pool.query(`
    SELECT id, store_id AS "storeId", item_name AS "itemName", sku,
           quantity, price::float AS price, notes,
           (quantity * price)::float AS "totalValue",
  ...
  store.items = stockRes.rows;
  store.totalUnits = store.items.reduce((s, it) => s + (Number(it.quantity) || 0), 0);
  store.totalValuation = store.items.reduce((s, it) => s + (Number(it.totalValue) || 0), 0);
  return store;
}

async function insertStore({ id, name, location, contact, notes }) {
  await initDB();
  const storeId = id || ("store_" + Date.now());
  const { rows } = await pool.query(`
    INSERT INTO stores (id, name, location, contact, notes)
    VALUES ($1, $2, $3, $4, $5)
    RETURNING id, name, location, contact, notes, created_at AS "createdAt"
  `, [storeId, name.trim(), location?.trim() || null, contact?.trim() || null, notes?.trim() || null]);
  return rows[0];
}

async function updateStore(id, { name, location, contact, notes }) {
  await initDB();
  const fields = [];
  const vals = [];
  let i = 1;
  if (name !== undefined) { fields.push(`name = $${i++}`); vals.push(name.trim()); }
  if (location !== undefined) { fields.push(`location = $${i++}`); vals.push(location?.trim() || null); }
  if (contact !== undefined) { fields.push(`contact = $${i++}`); vals.push(contact?.trim() || null); }
  if (notes !== undefined) { fields.push(`notes = $${i++}`); vals.push(notes?.trim() || null); }
  if (!fields.length) return null;
  vals.push(id);
  const { rows } = await pool.query(`
    UPDATE stores SET ${fields.join(", ")} WHERE id = $${i}
    RETURNING id, name, location, contact, notes, created_at AS "createdAt"
  `, vals);
  return rows[0] || null;
}
...
async function getStoreStock(storeId) {
  await initDB();
  const { rows } = await pool.query(`
    SELECT id, store_id AS "storeId", item_name AS "itemName", sku,
           quantity, price::float AS price, notes,
...
async function insertStoreStockItem({ id, storeId, itemName, sku, quantity, price, notes }) {
  await initDB();
  const itemId = id || ("sstk_" + Date.now());
  const { rows } = await pool.query(`
    INSERT INTO store_stock (id, store_id, item_name, sku, quantity, price, notes)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING id, store_id AS "storeId", item_name AS "itemName", sku,
              quantity, price::float AS price, notes,
              (quantity * price)::float AS "totalValue",
              created_at AS "createdAt", updated_at AS "updatedAt"
  `, [
...
async function updateStoreStockItem(id, { itemName, sku, quantity, price, notes }) {
  await initDB();
  const fields = [];
  const vals = [];
  let i = 1;
  if (itemName !== undefined) { fields.push(`item_name = $${i++}`); vals.push(itemName.trim()); }
  if (sku !== undefined) { fields.push(`sku = $${i++}`); vals.push(sku?.trim() || null); }
  if (quantity !== undefined) { fields.push(`quantity = $${i++}`); vals.push(Math.max(0, Number(quantity) || 0)); }
  if (price !== undefined) { fields.push(`price = $${i++}`); vals.push(Math.max(0, Number(price) || 0)); }
  if (notes !== undefined) { fields.push(`notes = $${i++}`); vals.push(notes?.trim() || null); }
  if (!fields.length) return null;
  fields.push(`updated_at = NOW()`);
  vals.push(id);
  const { rows } = await pool.query(`
    UPDATE store_stock SET ${fields.join(", ")} WHERE id = $${i}
    RETURNING id, store_id AS "storeId", item_name AS "itemName", sku,
              quantity, price::float AS price, notes,
              (quantity * price)::float AS "totalValue",
              created_at AS "createdAt", updated_at AS "updatedAt"
...
async function deleteStoreStockItem(id) {
  await initDB();
  await pool.query("DELETE FROM store_stock WHERE id = $1", [id]);
}

// ─── Bootstrap (Fast Consolidated Fetch) ──────────────────────────────────────
async function getBootstrapData() {
  await initDB();
  const [orders, stock, expenses, brandExpenses, revenue, stores] = await Promise.all([
    getOrders(),
    getStock(),
    getExpenses(),
    getBrandExpenses(),
  ...
  return { orders, stock, expenses, brandExpenses, revenue, stores };
}
...
module.exports = {
  ...
  // stores & physical shop stock
  getStores, getStoreById, insertStore, updateStore, deleteStore,
  getStoreStock, insertStoreStockItem, updateStoreStockItem, deleteStoreStockItem,
};